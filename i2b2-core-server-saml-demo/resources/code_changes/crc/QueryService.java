...
import src.server.edu.harvard.i2b2.crc.dao.SAMLUserDAO;
...
public class QueryService {
    ...
    public OMElement publishDataRequest(OMElement request) {
        ...
        try {
            String requestXml = setSamlUserToLocalUser(request.toString());
    ...
    /**
     * swc20160523 This function is main webservice interface to get the
     * I2B2HIVE.CRC_DB_LOOKUP data. It uses AXIOM elements(OMElement) to
     * conveniently parse xml messages.
     *
     * It accepts incoming request, and returns a response, both in i2b2 message
     * format.
     *
     * @param OMElement getAllDblookupsElement
    ...
    /**
     * swc20160523 This function is main webservice interface to get specific
     * I2B2HIVE.CRC_DB_LOOKUP data. It uses AXIOM elements(OMElement) to
     * conveniently parse xml messages.
     *
     * It accepts incoming request, and returns a response, both in i2b2 message
     * format.
     *
     * @param OMElement getDblookupElement
     * @return OMElement in i2b2message format
     * @throws Exception
     */
    ...
    /**
     * swc20160523 This function is main webservice interface to add a new entry
     * to the I2B2HIVE.CRC_DB_LOOKUP data. It uses AXIOM elements(OMElement) to
     * conveniently parse xml messages.
     *
     * It accepts incoming request, and returns a response, both in i2b2 message
     * format.
    ...
    /**
     * swc20160523 This function is main webservice interface to delete specific
     * I2B2HIVE.CRC_DB_LOOKUP data. It uses AXIOM elements(OMElement) to
     * conveniently parse xml messages.
     *
     * It accepts incoming request, and returns a response, both in i2b2 message
     * format.
     *
     * @param OMElement deleteDblookupElement
     * @return OMElement in i2b2message format
     * @throws Exception
     */
    ...
    private OMElement handleRequest(String requestType, OMElement request) {
        ...
        try {
            ...
            String response = requestHandlerDelegate.handleRequest(setSamlUserToLocalUser(request.toString()));
    ...
    private String setSamlUserToLocalUser(String requestXml) {
        try {
            ...
            if (requestMessageType.getMessageHeader() != null) {
                if (requestMessageType.getMessageHeader().getSecurity() != null) {
                    SecurityType credentials = requestMessageType.getMessageHeader().getSecurity();
                    String username = credentials.getUsername().trim();
                    if (username.startsWith("saml:")) {
                        String localUsername = SAMLUserDAO.getLocalUser(username);
                        requestXml = requestXml.replaceAll(username, localUsername);
                    }
                }
            }
        } catch (JAXBUtilException | I2B2Exception exception) {
            log.error("Fail to map SAML user to local user.", exception);
        }
